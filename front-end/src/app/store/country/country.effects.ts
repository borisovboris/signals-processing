...
import { map, exhaustMap, catchError, withLatestFrom, switchMap, tap } from 'rxjs/operators';
...
export class CountryEffects {
  ...
  this.actions$.pipe(
    ...
    switchMap(([_, countries]) =>
      this.countriesService.readCountriesWithOffset(countries[countries.length - 1].id!).pipe(
        map((countries) => CountryActions.additionalCountriesFetched({ countries })),
        catchError(() => EMPTY)
      )
    )
  )
